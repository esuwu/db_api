...
async function getForumUsers(data = {}) {
    try {
        let res;
        if (data.since) {
            res = await pool.query(`SELECT U.about, U.email, U.fullname, U.nickname FROM users AS U RIGHT OUTER JOIN forumusers AS FU
            ...
            res = pool.query(`SELECT U.about, U.email, U.fullname, U.nickname FROM users AS U RIGHT OUTER JOIN forumusers AS FU
                              ON FU.nickname = U.nickname
